    ...
    for i=1:10      
        ...
        for k = Kidx                        
            ...
            for n = 0:1:NAP-1
                ...
                s1 = Qm_inv(:,:,m)*S(:,k) ;
                s2 = (Qm_inv(:,:,m)'*S(:,k))' ;            
                N1 = real(s2*QY(:,:,m)*s1) ; %line 6 of Alg. 2 of [1]    
                N2 = real(s2*S(:,k)) ;            %line 6 of Alg. 2 of [1]                    
                am(n+1) = Beta(k,m)*N2 ;    %line 6 of Alg. 2 of [1]                 
                bm(n+1) = Beta(k,m)*N1 ;    %line 6 of Alg. 2 of [1]         
            end            
            c = compute_poly_coefficients(am,bm) ;             
            % Compute Roots
            r = roots(c) ;             
            % Pick Real Valued Roots and add boundary Condition 0
            Val = [r(imag(r)==0)' -1*gamma_hat(k)];  %line 9 of Alg. 2 of [1]        Added the boundary conditions              
            % Look for the minimizer
            f = zeros(1,length(Val)) ; 
            for n=1:1:NAP
                f1 = log(1+am(n)*Val) ;     
                f2 = Val*bm(n)./(1+am(n)*Val) ;
                f = f+f1-f2 ; %line 9 of Alg. 2 of [1] 
            end
            [~,idx] = min(f) ;    
            Val = Val(idx) ;            
            % Compute delta to Update 
            delta = max(Val,-1*gamma_hat(k)) ;   %c.f. line 6 of Alg. 1 of [1]
            % Update gamma_hat with delta
            gamma_hat(k) = gamma_hat(k) + delta ;%c.f. line 7/11 of Alg. 1/2 of [1]
            ...
            for m=1:M 
                ...
                N2 = real(s2*S(:,k)) ;                 
                N3 = s1*s2;
                N4 = 1 + (delta*Beta(k,m)*N2);  %c.f. line 13 of Alg. 2 of [1]
                q = (delta*Beta(k,m))/N4;           %c.f. line 13 of Alg. 2 of [1] 
                Qm_inv(:,:,m) = Qm_inv(:,:,m) - q*N3 ;  %c.f. line 13 of Alg. 2 of [1]
            end            
        end
